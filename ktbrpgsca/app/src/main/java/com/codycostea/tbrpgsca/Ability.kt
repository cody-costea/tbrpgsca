...
open class Ability(val id: Int, open val name: String, open val range: Boolean = false, open val steal: Boolean = false,
    ...
    open fun execute(user: Actor, targeted: Actor, applyCosts: Boolean): String {
        var s = ""
        var dmg = (Math.random() * 4).toInt()
        val target = if (targeted.reflect && this.dmgType == 3) user else targeted
        val trgResMap = target.res
        var res = if (trgResMap === null) 3 else (trgResMap[this.elm] ?: 3)
        if (res > 6) res = -1
        when (this.dmgType) {
            1 -> dmg += (this.atkI + (user.def + user.atk) / 2) / (target.def * res + 1)
            2 -> dmg += (this.atkI + user.wis) / (target.spi * res + 1)
            3 -> dmg += (this.atkI + user.spi) / (target.wis * res + 1)
            4 -> dmg += (this.atkI + (user.agi + user.atk) / 2) / ((target.agi + target.def) / 2 * res + 1)
            5 -> dmg += (this.atkI + (user.wis + user.atk) / 2) / ((target.spi + target.def) / 2 * res + 1)
            6 -> dmg += (this.atkI + (user.agi + user.wis + user.atk) / 3) / ((target.agi + target.spi) / 2 * res + 1)
            7 -> dmg += (this.atkI + (user.spi + user.atk + user.def) / 3) / ((target.wis + target.def) / 2 * res + 1)
            else -> dmg += (this.atkI + user.atk) / (target.def * res + 1)
        }
        if (this.dmgType == 2
                || this.dmgType == 3
                //|| this.qty > 0 && this.mQty < 1
                || this.dmgType != 4 && (Math.random() * 13 + user.agi / 5).toInt() > 2 + target.agi / 4
                || this.dmgType == 4 && (Math.random() * 13 + user.agi / 3).toInt() > 2 + target.agi / 4) {
            ...
            if (aStates != null) {
                for (state in aStates) state.inflict(target, false)
            }
            val trgStates = target.states
            if (trgStates != null) {
            ...
            val trgItems = target.items
            if (this.steal
                    && trgItems !== null
                    && trgItems !== user.items
                    && trgItems.isNotEmpty()
                    && (Math.random() * 12 + user.agi / 4).toInt() > 4 + target.agi / 3) {
